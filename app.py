...
st.markdown(
    f"<div class='terminal-header'>TERMINAL BUILD v5.5.0 | SYS_TIME: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | MOBILE OPTIMIZED</div>",
    unsafe_allow_html=True
)
            ...
            if not api_key:
                ...
                if not quote:
                    ...
                    if df_kline is None or len(df_kline) < 15:
                        st.warning("获取到的有效 K 线极少，仅能通过最新行情管中窥豹。这可能是新股或数据源暂时熔断。")
                        with st.spinner("🧠 首席策略官撰写基础资产评估报告..."):
                            ...
                            【由于行情数据缺失，请重点进行：估值分析、所属板块资金博弈推演】
                            请输出：1. 基本面诊断 2. 纯量价资金意图盲猜 3. 风险提示
                            """
                        ...
                        # ---> 重点优化 Prompt：强制 AI 进行【资金面】与【支撑压力】分析 <---
                        with st.spinner("🧠 首席策略官进行多维深度解构(基本面+资金面+技术面)..."):
...
请对股票 {name}({symbol_input}) 做一份极具实战价值的【估值 + 资金流 + 支撑/压力】综合研判。
...
2. 🌊 **资金面穿透**：结合今日换手率（{quote['turnover']}%）、量能状态（{tech['vol_state']}）以及近期结构突破情况，推演主力机构是在悄悄吸筹、接力洗盘、还是在派发跑路？
3. 🎯 **精准支撑与压力位**：
   - 结合 EMA20 和多头 FVG 给出具体的**短线强支撑位**（具体价格）。
   - 结合近期高点或空头 FVG 给出具体的**上行重压位**（具体价格）。
4. ⚔️ **兵棋推演与操作策略**：
   - 当前是属于右侧追涨、左侧潜伏还是观望区？
   - 给出明确的“如果跌破 XX 必须止损”或“突破 XX 可看高一线”的交易逻辑。
结论定调：[看多 / 观察 / 谨慎 / 偏空]
"""
                            st.markdown(call_ai(prompt))
